...
class SNAKE:
    def __init__(self):
        ...
        self.body_tr = pygame.transform.scale(
            self.body_tr, (cell_size, cell_size))
        self.body_tl = pygame.transform.scale(
            self.body_tl, (cell_size, cell_size))
        self.body_br = pygame.transform.scale(
            self.body_br, (cell_size, cell_size))
        self.body_bl = pygame.transform.scale(
            self.body_bl, (cell_size, cell_size))
    ...
    def change_direction(self):
        if movements_queued:
            self.direction = movements_queued.pop(0)
...
class MAIN:
    ...
    def draw_score(self):
        ...
        score_x = cell_size * cell_number - round(cell_size * 1.5)
        score_y = cell_size * cell_number - cell_size
        score_rect = score_surface.get_rect(center=(score_x, score_y))
        apple_rect = apple.get_rect(
            midright=(score_rect.left, score_rect.centery))
        bg_rect = pygame.Rect(apple_rect.left, apple_rect.top,
                              apple_rect.width + score_rect.width + cell_size * 0.15, apple_rect.height)
...
game_font = pygame.font.Font(
    'Font/PoetsenOne-Regular.ttf', round(cell_size * 0.625))
movements_queued = []
        ...
        if event.type == pygame.KEYDOWN:
            if event.key == pygame.K_UP:
                if main_game.snake.direction.y != 1:
                    movements_queued.append(Vector2(0, -1))
            if event.key == pygame.K_DOWN:
                if main_game.snake.direction.y != -1:
                    movements_queued.append(Vector2(0, 1))
            if event.key == pygame.K_RIGHT:
                if main_game.snake.direction.x != -1:
                    movements_queued.append(Vector2(1, 0))
